...
function SheetContent({
  ...
  /** Set false for a non-modal sheet that leaves the page behind it live. */
  showOverlay?: boolean
}) {
  return (
    <SheetPortal>
      {showOverlay && <SheetOverlay />}
      {/* `side` is physical (`left-0` / `right-0`), so the enter/exit offsets
          stay physical too. The registry's `rtl:` translate overrides flipped
          only the animation, making a right-pinned sheet slide in from the
          left under `dir="rtl"`; they are removed here. */}
